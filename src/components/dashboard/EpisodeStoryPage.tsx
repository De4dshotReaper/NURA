...
export const EpisodeStoryPage: React.FC<EpisodeStoryPageProps> = ({ episodeId, userId, onBack, showBackButton = true }) => {
  ...
  return <div className="max-w-4xl mr-auto space-y-9 pb-16">
    ...
    {isLoading ? <div className="rounded-[1.75rem] border border-gray-100 bg-white p-10 text-center" aria-busy="true"><p className="text-sm text-nuraTextSecondary">{t('episodes.loadingStory')}</p></div> : error ? <div className="rounded-[1.75rem] border border-red-100 bg-white p-8 text-center" role="alert"><p className="text-sm text-red-700">{error}</p></div> : !episode || !symptom ? <div className="rounded-[1.75rem] border border-gray-100 bg-white p-10 text-center"><p className="text-sm text-nuraTextSecondary">{t('episodes.notFound')}</p></div> : <>
      ...
      {featureFlags.visitPacket && <section className="space-y-4 rounded-[1.75rem] border border-gray-100 bg-white p-4" aria-labelledby="prepare-for-visit-title">
        <div><h2 id="prepare-for-visit-title" className="font-heading text-xl font-extrabold text-nuraText">{t('visitPacket.prepareForVisit')}</h2><p className="mt-1 text-sm text-nuraTextSecondary">{t('visitPacket.prepareDescription')}</p></div>
        {!visitPacketPreview ? <button type="button" onClick={() => void inspectVisitPacket(false)} disabled={isBuildingVisitPacket} aria-busy={isBuildingVisitPacket} className="rounded-xl bg-primary px-4 py-2.5 text-sm font-semibold text-white hover:bg-blue-600 disabled:cursor-not-allowed disabled:opacity-50">{isBuildingVisitPacket ? t('visitPacket.buildingPacket') : t('visitPacket.preparePacket')}</button> : <>
          <div className="rounded-xl border border-gray-100 bg-gray-50 p-3"><p className="font-semibold text-nuraText">{t('visitPacket.packetReady')}</p><p className="mt-1 text-xs font-bold uppercase tracking-wider text-nuraTextSecondary">{t('visitPacket.includes')}</p>{visitPacketReadiness.length > 0 && <ul className="mt-2 flex flex-wrap gap-x-5 gap-y-1 text-sm text-nuraText">{visitPacketReadiness.map((item) => <li key={item} className="before:mr-1.5 before:content-['•']">{item}</li>)}</ul>}</div>
          <div className="flex flex-wrap gap-2">
            <button ref={previewButtonRef} type="button" onClick={() => setIsVisitPacketPreviewOpen(true)} disabled={isPdfBusy} className="inline-flex items-center gap-2 rounded-xl border border-gray-200 bg-white px-4 py-2.5 text-sm font-semibold text-nuraTextSecondary hover:bg-blue-50 disabled:cursor-not-allowed disabled:opacity-50"><Eye className="h-4 w-4" />{t('visitPacket.previewPacket')}</button>
            <button type="button" onClick={() => void handleVisitPacketDownload()} disabled={isPdfBusy} aria-busy={isGeneratingPdf} className="inline-flex items-center gap-2 rounded-xl bg-primary px-4 py-2.5 text-sm font-semibold text-white hover:bg-blue-600 disabled:cursor-not-allowed disabled:opacity-50"><Download className="h-4 w-4" />{isGeneratingPdf ? t('visitPacket.preparingPdf') : t('visitPacket.downloadPdf')}</button>
            <button type="button" onClick={() => void handleVisitPacketShare()} disabled={isPdfBusy || isNativeShareUnavailable} aria-busy={isSharingPdf} className="inline-flex items-center gap-2 rounded-xl border border-gray-200 bg-white px-4 py-2.5 text-sm font-semibold text-nuraTextSecondary hover:bg-blue-50 disabled:cursor-not-allowed disabled:opacity-50"><Share2 className="h-4 w-4" />{isSharingPdf ? t('visitPacket.preparingPdf') : t('visitPacket.sharePacket')}</button>
          </div>
          {isNativeShareUnavailable && <p className="text-sm text-nuraText" role="status">{t('visitPacket.shareUnavailable')}</p>}
        </>}
