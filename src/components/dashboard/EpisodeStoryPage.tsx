  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (!mounted) return;
        if (episodeError) { console.error('Failed to load selected health episode:', episodeError); setError('Nura couldn’t load this health episode.'); return; }
        ...
        if (!mounted) return;
        if (symptomError || !symptomRow) { console.error('Failed to load episode initial symptom:', symptomError); setError('Nura couldn’t load this episode’s initial symptoms.'); return; }
        ...
        const normalized: TimelineEventCardData[] = [{ id: `symptom-${symptomRow.id}`, title: 'Symptoms Recorded', description: symptomRow.symptoms, timestamp: symptomRow.created_at, icon: Activity, badge: symptomRow.severity === null ? undefined : `Severity ${symptomRow.severity}/10`, badgeColor: 'bg-blue-50 text-blue-700 border-blue-200/60', relationshipLabel: 'Episode start', details: [`Duration: ${symptomRow.duration || 'Not recorded'}`] }];
        followUps.forEach((item) => normalized.push({ id: `followup-${item.id}`, title: 'Follow-up Recorded', description: item.current_symptoms?.trim() || item.progress?.trim() || 'Follow-up information recorded', timestamp: item.created_at, icon: CheckCircle2, badge: item.progress || undefined, badgeColor: 'bg-teal-50 text-teal-700 border-teal-200/60', relationshipLabel: 'Follow-up', details: [`Medicine compliance: ${item.medicine_compliance || 'Not recorded'}`, item.medicine_reason ? `Medicine note: ${item.medicine_reason}` : '', `Side effects: ${item.has_side_effects ? item.side_effects_text || 'Reported' : 'No'}`, item.questions ? `Questions: ${item.questions}` : ''].filter(Boolean) }));
        consultations.forEach((item) => { const timestamp = item.consultation_at && !Number.isNaN(new Date(item.consultation_at).getTime()) ? item.consultation_at : item.created_at; normalized.push({ id: `consultation-${item.id}`, title: 'Appointment Recorded', description: [item.doctor_name, item.clinic_name].filter(Boolean).join(' • ') || 'Consultation details recorded', timestamp, icon: Calendar, badge: 'Consultation', badgeColor: 'bg-violet-50 text-violet-700 border-violet-200/60', relationshipLabel: 'For episode', details: [item.notes, `Follow-up recommended: ${item.follow_up_recommended ? 'Yes' : 'No'}`, item.follow_up_notes || ''].filter(Boolean) }); });
        ...
        questionCycles.forEach((cycle, key) => { cycle.sort((a, b) => new Date(a.created_at).getTime() - new Date(b.created_at).getTime()); normalized.push({ id: `questions-${key}`, title: 'Questions Prepared', description: key === 'first' ? 'Prepared before the first consultation' : 'Prepared after the previous consultation', timestamp: cycle[0].created_at, icon: HelpCircle, badge: `${cycle.length} ${cycle.length === 1 ? 'Question' : 'Questions'}`, badgeColor: 'bg-amber-50 text-amber-700 border-amber-200/60', relationshipLabel: key === 'first' ? 'First consultation' : 'Next consultation', details: cycle.map((item) => item.question) }); });
        prescriptions.forEach((item) => normalized.push({ id: `prescription-${item.id}`, title: 'Prescription Added', description: item.file_name, timestamp: item.uploaded_at, icon: Pill, badge: 'Prescription', badgeColor: 'bg-emerald-50 text-emerald-700 border-emerald-200/60', relationshipLabel: 'Linked consultation', details: ['Explicitly linked to a consultation in this episode'], actionLabel: 'View Prescription', onAction: () => setPrescriptionDetail(item) }));
        labs.forEach((item) => normalized.push({ id: `lab-${item.id}`, title: 'Lab Report Uploaded', description: item.report_type || item.file_name, timestamp: item.uploaded_at, icon: FileText, badge: 'Lab Report', badgeColor: 'bg-purple-50 text-purple-700 border-purple-200/60', relationshipLabel: 'Linked consultation', details: ['Explicitly linked to a consultation in this episode'], actionLabel: 'View Lab Report', onAction: () => setLabDetail(item) }));
        setEvents(normalized);
      } catch (loadError) { if (mounted) { console.error('Unexpected error loading episode story:', loadError); setError('Nura couldn’t load this episode story.'); } }
      finally { if (mounted) setIsLoading(false); }
    };
    void load(); return () => { mounted = false; };
  }, [episodeId, userId]);

  if (prescriptionDetail) return <PrescriptionSummaryPage onBack={() => setPrescriptionDetail(null)} prescriptionTitle={prescriptionDetail.file_name} uploadDate={formatDate(prescriptionDetail.uploaded_at)} fileType={prescriptionDetail.file_type} medicines={Array.isArray(prescriptionDetail.medicines) ? prescriptionDetail.medicines as ExtractedMedicine[] : []} />;
  if (labDetail) { const analysis: LabReportAnalysis = { reportFormat: 'structured', reportType: labDetail.report_type, laboratory: labDetail.laboratory, reportDate: labDetail.report_date, rawText: labDetail.raw_text, parameters: Array.isArray(labDetail.parameters) ? labDetail.parameters as LabReportAnalysis['parameters'] : [] }; return <LabReportSummaryPage onBack={() => setLabDetail(null)} reportTitle={labDetail.file_name} uploadDate={formatDate(labDetail.uploaded_at)} fileType={labDetail.file_type} analysisData={analysis} />; }

  const groups: EventGroup[] = Array.from(events.reduce<Map<string, TimelineEventCardData[]>>((map, event) => { const key = localDateKey(event.timestamp); map.set(key, [...(map.get(key) ?? []), event]); return map; }, new Map()).entries()).map(([dateKey, groupEvents]) => ({ dateKey, dateLabel: formatDate(groupEvents[0].timestamp), events: groupEvents.sort((a, b) => new Date(a.timestamp).getTime() - new Date(b.timestamp).getTime()) })).sort((a, b) => b.dateKey.localeCompare(a.dateKey));

  return <div className="max-w-4xl mr-auto space-y-9 pb-16">
    <button type="button" onClick={onBack} className="inline-flex items-center gap-2 text-sm font-semibold text-nuraTextSecondary hover:text-primary transition-colors group"><ArrowLeft className="h-4 w-4 group-hover:-translate-x-1 transition-transform" />Back to Health Episodes</button>
    {isLoading ? <div className="rounded-[1.75rem] border border-gray-100 bg-white p-10 text-center" aria-busy="true"><p className="text-sm text-nuraTextSecondary">Loading episode story...</p></div> : error ? <div className="rounded-[1.75rem] border border-red-100 bg-white p-8 text-center" role="alert"><p className="text-sm text-red-700">{error}</p></div> : !episode || !symptom ? <div className="rounded-[1.75rem] border border-gray-100 bg-white p-10 text-center"><p className="text-sm text-nuraTextSecondary">Health episode not found.</p></div> : <>
      <header className="space-y-4"><div className="inline-flex rounded-full bg-blue-50/80 px-3 py-1 text-xs font-semibold tracking-wider text-primary">HEALTH EPISODE</div><div className="flex flex-wrap items-start justify-between gap-4"><div className="space-y-2"><h1 className="font-heading text-3xl sm:text-4xl font-extrabold text-nuraText">Health Episode</h1><p className="max-w-2xl whitespace-pre-line text-lg font-semibold text-nuraText">{symptom.symptoms}</p></div><span className={`rounded-full px-3 py-1 text-[11px] font-bold uppercase tracking-wider ${episode.status === 'active' ? 'bg-emerald-50 text-emerald-700' : 'bg-gray-100 text-gray-600'}`}>{episode.status}</span></div><p className="text-sm text-nuraTextSecondary">Started {formatDate(episode.started_at)}{episode.status === 'completed' && episode.completed_at ? ` • Completed ${formatDate(episode.completed_at)}` : ''}</p></header>
      <div className="space-y-10">{groups.map((group) => <section key={group.dateKey} className="space-y-4"><div className="flex items-center gap-3"><div className="rounded-xl border border-blue-200/50 bg-blue-100/80 px-3.5 py-1.5 font-heading text-sm font-extrabold text-primary">{group.dateLabel}</div><div className="h-px flex-1 bg-gray-100" /></div><div className="space-y-3">{group.events.map((event, index) => <TimelineEventCard key={event.id} event={event} expanded={expandedId === event.id} isLastInGroup={index === group.events.length - 1} onToggle={() => setExpandedId((current) => current === event.id ? null : event.id)} formatDate={formatDate} formatTime={formatTime} />)}</div></section>)}</div>
    </>}
  </div>;
};
